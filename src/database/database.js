...
class DB {
  ...
  async listUsers(page = 1, limit = 10, name = '*') {
    ...
    try {
      ...
      const users = await this.query(connection, `SELECT id, name, email FROM user WHERE name LIKE ? LIMIT ${limit + 1} OFFSET ${offset}`, [nameFilter]);
